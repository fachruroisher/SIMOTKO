<form action="<?= site_url('/').$access ?>" method="post" enctype="multipart/form-data" accept-charset="utf-8" class="form-horizontal martop" id="block-validate">
    <div class="form-group">
        <label class="control-label col-lg-3">Nama Shift</label>
        <div class="col-lg-7">
            <input type="hidden" name="kode" value="<?=$kode?>"/>
            <input type="text" class="form-control" placeholder="Example: pagi, siang dll" name="shift" value="<?=$shift?>" required/>
        </div>
    </div> 
    <div class="form-group">
    ...
    <div class="form-actions">
        ...
        <?php if($form=='edit'){ ?>
            <a href="<?=site_url('sf_ad')?>" class="btn btn-warning">Cancel</a>
        <?php } ?>
    </div>
</form>  
